 ...
 * Plugin URI: http://lovelyideas.net/
 * Description: Quickly create a maintenance/coming-soon page. Use this plugin whenever your site is down for maintenance or undergoing development.

 * Author: Chirs Mark 
 * Author URI: http://lovelyideas.net/
 * Version: 1.0.0
 *
 */
if ( ! defined( 'ABSPATH' ) ) exit;

if( !class_exists( 'Maintenance_Mode' ) ) {
	/**
	 * Main Class
	 *
	 * @since 1.0
	...
	class Maintenance_Mode {
		...
		/**
		 * Maintenance Mode theme options Object
		 *
		 * @var object
		 * @since 1.0
		 */
		...
		Private function __construct() {
 		...
 		/**
		 * Main Maintenance Mode Instance
		 *
		 * Insures that only one instance of Maintenance_Mode exists in memory at any one
		 * time. In singleton class you cannot create a second instance
		 *
		 * @since 1.0
		 * @static
		 * @staticvar array $instance
		 * @uses Maintenance_Mode::setup_constants() Setup the contants needed
	 	 * @uses Maintenance_Mode::includes() Include the required files
	 	 * @uses Maintenance_Mode::setup_actions() Setup the hooks and actions
	 	 * @uses Maintenance_Mode::load_textdomain() loads the textdomain for tranlation
		 * @see mp()
		 * @return The one true Maintenance_Mode
		 */
		...
		private function setup_actions() {
			...
			register_activation_hook( $this->file , array( 'Maintenance_Mode_Activator', 'activate' ) );
			...
			if( $this->setting_options->get_option( 'mp_activate','mp_basics' ) =='on' ) {
				...
				if( !is_user_logged_in() ) {
					if( !$this->check_excluded_page() ) {
						...
						$file = $this->plugin_dir . 'public/template/maintenance-mode-display.php';
	        			include($file);
	         			exit();
	         		}
	         	}
		...
		function admin_bar_notice() {
		    ...
		    if( $value == 'on') {
		    	$argsParent=array(
			        'id' => 'admin_bar_notice',
			        'title' => __( 'Maintenance Mode On','maintenance-page' ),
			        'href' => admin_url('options-general.php?page=mm_settings'),
			    ...
			    $argsParent=array(
			        'id' => 'admin_bar_notice',
			        'title' => __( 'Maintenance Mode Off','maintenance-page' ),
			        'href' => admin_url('options-general.php?page=mm_settings'),
      ...
      public function create_subscribe_table_mu( $blog_id, $user_id, $domain, $path, $site_id, $meta ) {
         if ( is_plugin_active_for_network( 'maintenance-page/maintenance-page.php' ) ) {
            ...
            Maintenance_Mode_Activator::create_subscribe_table();
...
/**
 * The main function responsible for returning the one true Maintenance_Mode
 ...
 *
 * @since 1.0
 * @return object The one true Maintenance_Mode Instance
 */
function MP() {
	return Maintenance_Mode::getInstance();
}
